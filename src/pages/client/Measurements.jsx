      ...
      <div className="p-4 pb-10 space-y-4">
        ...
        {!showForm ? (
          ...
          <Card className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {FIELDS.map(f => (<div key={f.key} className="bg-dark-900 rounded-xl px-3 py-2"><p className="text-gray-500 text-xs mb-1">{f.label} ({f.unit})</p><input type="number" inputMode="decimal" step="0.1" min="0" value={form[f.key]} onChange={e => setForm(ff => ({...ff, [f.key]: e.target.value}))} placeholder={f.placeholder} style={inputStyle} className="w-full bg-transparent text-white text-sm focus:outline-none placeholder-gray-700" /></div>))}
            </div>
