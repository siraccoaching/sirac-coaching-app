...
import { Dumbbell, Clock, Plus, Trash2, CheckCircle, ChevronDown, ChevronUp, ExternalLink, X } from 'lucide-react'

// Fix Safari autofill white background issue
const inputStyle = {
...
export default function SessionLogger() {
  ...
  // logs: { [exercise_id]: { sets: [{reps, load, rpe, _done}], notes: '' } }
  ...
  async function loadSession() {
    ...
    if (data?.program_exercises) {
      ...
      for (const ex of data.program_exercises.sort((a,b) => a.order_index - b.order_index)) {
        initial[ex.id] = {
          sets: Array.from({ length: ex.sets }, () => ({ reps: ex.reps || '', load: ex.load || '', rpe: '', _done: false })),
          notes: '',
        }
  ...
  function addSet(exId, ex) {
    setLogs(l => ({
      ...l,
      [exId]: { ...l[exId], sets: [...l[exId].sets, { reps: ex.reps || '', load: ex.load || '', rpe: '', _done: false }] }
    }))
  }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="bg-dark-800 border border-white/10 rounded-3xl p-6 w-full max-w-sm space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowQuitConfirm(false)} className="text-gray-500 hover:text-white">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowQuitConfirm(false)}
                  className="flex-1 py-3 border border-white/10 text-gray-300 rounded-2xl text-sm font-medium">
                  Continuer
                </button>
                <button onClick={() => navigate('/client/program', { replace: true })}
                  className="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white rounded-2xl text-sm font-medium transition-colors">
                  Quitter
                </button>
              </div>
            </div>
