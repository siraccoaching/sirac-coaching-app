...
export default function JoinCoach() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', password: '', sport: '', position: '' })
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('id,full_name,avatar_url').eq('invite_code', code).single()
  ...
  if (!code || !coach) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a14', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 320 }}>
        ...
        <p style={{ color: '#888', fontSize: 14 }}>Ce lien d\'invitation est invalide ou a expiré.</p>
      </div>
    </div>
  )
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a14', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 340 }}>
        ...
        <p style={{ color: 'white', fontSize: 20, fontWeight: 700, marginBottom: 10 }}>Compte créé !</p>
        <p style={{ color: '#a78bfa', fontSize: 14, marginBottom: 8 }}>Bienvenue dans l\'équipe de {coach.full_name}.</p>
        <p style={{ color: '#888', fontSize: 13 }}>Vérifie ta boîte mail pour confirmer ton adresse, puis connecte-toi.</p>
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a14', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: '0 0 6px' }}>Rejoindre {coach.full_name}</p>
          <p style={{ color: '#888', fontSize: 13, margin: 0 }}>Crée ton compte athlète pour accéder à ton programme</p>
        </div>

        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={labelStyle}>Prénom et nom</label>
            <input style={inputStyle} placeholder="Jean Dupont" value={form.full_name}
          ...
          <p style={{ textAlign: 'center', color: '#666', fontSize: 12, margin: 0 }}>
            Déjà un compte ?{' '}
            <span onClick={() => navigate('/login')} style={{ color: '#a78bfa', cursor: 'pointer' }}>Se connecter</span>
          </p>
        </form>
      </div>
    </div>
