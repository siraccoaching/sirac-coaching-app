...
import { Layers, Dumbbell, Clock, ChevronDown, ChevronUp, Users, Trash2 } from 'lucide-react'
...
export default function ProgramDetail() {
  ...
  return (
    <PageLayout title={prog.name} back="/coach/programs" action={
      <button onClick={handleDelete} className="p-2 text-gray-500 hover:text-red-400 transition-colors"><Trash2 size={17} /></button>
    }>
      <div className="p-4 pb-10 space-y-4">
        ...
        {isBlock && blocks.map((block, bi) => (
          <Card key={block.id} className="overflow-hidden">
            <button onClick={() => setOpenBlocks(ob => ({ ...ob, [bi]: !ob[bi] }))}
              ...
              <span className="text-gray-500 text-xs">{block.duration_weeks} sem - {block.program_sessions?.length || 0} seances</span>
            </button>
            {openBlocks[bi] && (
              <div className="p-3 space-y-2">
                {block.description && <p className="text-gray-500 text-xs px-1 mb-2">{block.description}</p>}
                {block.program_sessions?.sort((a,b) => a.order_index - b.order_index).map(sess => (
                  <SessionView key={sess.id} sess={sess} open={openSessions[sess.id]}
        ...
        {!isBlock && (
          <div className="space-y-2">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-1">Seances</h3>
            {simpleSessions.map(sess => (
              <Card key={sess.id} className="overflow-hidden">
                <SessionView sess={sess} open={openSessions[sess.id]}
                  onToggle={() => setOpenSessions(os => ({ ...os, [sess.id]: !os[sess.id] }))} />
              </Card>
            ))}
          </div>
          ...
          {exercises.map(ex => (
            <div key={ex.id} className="bg-dark-800 border border-white/5 rounded-lg p-2.5">
              ...
              <div className="flex flex-wrap gap-2">
                <Pill label="Series" value={ex.sets} />
                <Pill label="Reps" value={ex.reps} />
                {ex.load && <Pill label="Charge" value={ex.load} />}
                {ex.rpe && <Pill label="RPE" value={ex.rpe} />}
                {ex.rest_seconds > 0 && <Pill label="Recup" value={ex.rest_seconds + 's'} icon={<Clock size={9} />} />}
              </div>
