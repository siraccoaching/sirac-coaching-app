...
export default async function handler(req, res) {
  ...
  const { email, name, sport, position, phase } = req.body

  if (!email || !name) {
        return res.status(400).json({ error: 'Email et nom requis.' })
  }

  // Invite the user by email — they'll receive a link to set their own password
  ...
  return res.status(200).json({ user_id: inviteData.user.id, success: true })
}
